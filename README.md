![Nuget](https://img.shields.io/nuget/v/ag.DbData.SqlServer)
...
- [ag.DbData.SqlServer](https://www.nuget.org/packages/ag.DbData.SqlServer/)
...
```csharp
IServiceCollection AddAgSqlServer(this IServiceCollection services)
```

Appends the registration of ```IDbDataFactory``` and ```IDbDataObject``` to ```IServiceCollection```.

```csharp
IServiceCollection AddAgSqlServer(this IServiceCollection services, IConfigurationSection configurationSection)
```
...
```csharp
IServiceCollection AddAgSqlServer(this IServiceCollection services, Action<DbDataSettings> configureOptions)
```
